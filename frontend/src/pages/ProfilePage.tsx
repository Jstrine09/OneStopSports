...
export default function ProfilePage() {
  ...
  if (!isAuthenticated) {
    return (
      <div className="flex flex-col items-center gap-4 py-16">
        ...
        <p className="text-slate-500 dark:text-slate-400">Sign in to save your favourite teams and players</p>
        <button
  ...
  return (
    <div className="space-y-5">
      ...
      {/* User card */}
      <div className="flex items-center gap-3 rounded-xl bg-white px-4 py-4 dark:bg-slate-800">
        <div className="flex h-12 w-12 items-center justify-center rounded-full bg-blue-500/20 text-blue-400">
          <User size={24} />
        </div>
        <div>
          <p className="font-semibold">{username}</p>
          <p className="text-xs text-slate-500 dark:text-slate-400">OneStopSports member</p>
        </div>
      </div>
      ...
      <section className="space-y-2">
        <div className="flex items-center gap-2 px-1">
          ...
          <h2 className="text-sm font-semibold text-slate-600 dark:text-slate-300">Favourite Teams</h2>
          {favTeams.length > 0 && (
            <span className="text-xs text-slate-400 dark:text-slate-500">({favTeams.length})</span>
          )}
        </div>
        ...
        {loadingTeams ? (
          ...
          <p className="rounded-xl bg-white py-6 text-center text-sm text-slate-500 dark:bg-slate-800 dark:text-slate-400">
            ...
            {favTeams.map((team) => (
              <div key={team.id} className="relative">
                ...
                <button
                  ...
                  className="absolute right-1 top-1 flex h-[30px] w-[30px] items-center justify-center rounded-full bg-slate-100 text-slate-500 transition hover:bg-red-500/20 hover:text-red-400 dark:bg-slate-700 dark:text-slate-400"
      ...
      <section className="space-y-2">
        <div className="flex items-center gap-2 px-1">
          <Heart size={16} className="fill-red-500 text-red-500" />
          <h2 className="text-sm font-semibold text-slate-600 dark:text-slate-300">Favourite Players</h2>
          {favPlayers.length > 0 && (
            <span className="text-xs text-slate-400 dark:text-slate-500">({favPlayers.length})</span>
          )}
        </div>
          ...
          <div className="space-y-1">
            {favPlayers.map((player) => (
              ...
              >
                ...
                <span className="w-6 text-right text-xs font-bold text-slate-400 dark:text-slate-500">
                  {player.jerseyNumber ?? '—'}
                </span>
                ...
                <button
                  ...
                  className="flex h-[44px] w-[44px] items-center justify-center rounded-full text-slate-400 transition hover:bg-red-500/20 hover:text-red-400 dark:text-slate-500"
                  aria-label="Remove from favourites"
                >
                  <X size={14} />
                </button>
      ...
      <button
        ...
        className="flex w-full items-center gap-2 rounded-xl bg-white px-4 py-3 text-sm text-red-400 transition hover:bg-slate-50 dark:bg-slate-800 dark:hover:bg-slate-700"
      >
        <LogOut size={16} />
        Sign out
      </button>
