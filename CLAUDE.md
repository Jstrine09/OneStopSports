...
> **Last refreshed:** reflects commit `b07af4c` — the app is now **publicly deployed** (Vercel frontend + Render backend) with CORS/WS origins locked down to the real deploy domains. Render's free-tier cold starts are mitigated by an external UptimeRobot monitor (the in-repo GitHub keep-alive workflow was removed — see Infra (prod)). No code has changed since `4445393`; this refresh fixed stale migration-count (V1–V7 → V1–V9) and test-coverage-table facts in `.planning/cowork/OVERVIEW.md`, `ARCHITECTURE.md`, and `ROADMAP.md`. If you're picking this up in a new chat, this file + `.planning/cowork/` are the authoritative current-state context.
