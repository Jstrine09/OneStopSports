...
> **Last refreshed:** reflects commit `7bcb629` — the app remains **publicly deployed** (Vercel frontend + Render backend) with CORS/WS origins locked down to the real deploy domains. Render's free-tier cold starts are mitigated by an external UptimeRobot monitor (the in-repo GitHub keep-alive workflow was removed — see Infra (prod)). Since `6714a50` ingested the `.planning/` GSD setup, **Phase 1 (Backend Service Test Coverage) and Phase 2 (Postgres Migration Integration Tests) of the "v1 Harden & Test" milestone are both complete** (7/7 plans total): Phase 1 grew the backend suite from 9 test classes/66 tests to 17 test classes/120 tests, closing every previously-untested service (`NflApiService`, `ExternalApiService`, `ApiFootballService`, `BallDontLieService`, `UserService`, `SportService`, `PlayerService` photo/toDto/search, `GlobalExceptionHandler`); Phase 2 added `PostgresMigrationIT` (13 tests, opt-in via `mvn verify -Pintegration`) proving the V8/V9 Flyway migrations against a real Postgres. **Five further QA hardening fixes have since landed** (`0e0b5f5`, `ff9fc60`, `71e74b7`, `9a38f89`, `7bcb629`): unmapped backend routes now return a clean 404 instead of the 500 catch-all; Swagger/OpenAPI is disabled in the prod profile (QA S2); football player headshots now derive from the API-SPORTS media CDN once a player's stats have been looked up once, with a graceful initials-tile fallback in the UI (QA U1); `/api/auth/login`/`register` are rate-limited per-IP (+ per-username on login), returning 429 + `Retry-After` (QA S1); and the JWT model was hardened to a short-lived in-memory access token + httpOnly rotating refresh cookie with DB-backed revocation/reuse-detection, plus a response-wide Content-Security-Policy header (QA S3) — the backend suite is now **20 test classes/148 tests** (`mvn test`). The milestone is now on **Phase 3 — Frontend Test Foundation** (`status: verifying`, 2/4 phases complete, not yet started). If you're picking this up in a new chat, this file + `.planning/cowork/` + `.planning/ROADMAP.md`/`STATE.md` are the authoritative current-state context.
...
- **Headshots** (QA U1): `PlayerService.resolvePhotoUrl` — three layers: persisted `photoUrl` → a derived CDN URL keyed off `externalId` + sport → null. NBA/NFL derive the ESPN CDN URL (`a.espncdn.com/i/headshots/{nba|nfl}/players/full/{id}.png`) with no DB column needed; **football now also derives** a URL from the API-SPORTS media CDN (`media.api-sports.io/football/players/{apiSportsId}.png`) once `externalId` has been populated (lazily, on first career-stats lookup — see below), closing the earlier "football headshots unsupported" gap. Footballers who've never had their stats page opened still fall through to layer 3 (tracked as HARD-04). The frontend (`PlayerDetailPage`) now renders a graceful **initials-on-a-coloured-tile fallback** when no photo URL resolves, instead of a broken image icon.
...
**Further hardening fixes landed after that sweep:**
...
- ✅ **Football headshots + fallback tile** (commit `71e74b7`, QA finding U1) — football player photos now derive from the API-SPORTS media CDN (see Headshots above); the frontend falls back to an initials tile instead of a broken image icon.
- ✅ **Auth rate limiting** (commit `9a38f89`, QA finding S1) — `/api/auth/login`/`register` throttled per-IP (+ per-username on login), returning 429 + `Retry-After` (see Security → Auth rate limiting above).
- ✅ **Token-security hardening** (commit `7bcb629`, QA finding S3) — CSP header on every response, access token moved out of localStorage into memory, httpOnly rotating refresh cookie with DB-backed revocation + reuse detection, real logout (see Security → Token model above).
